...
import com.lonevox.grapplinghookneo.enchantments.ModEnchantments;
...
//				player.playSound(new SoundEvent(ResourceLocation.fromNamespaceAndPath("grappling_hook_neo", "enderstaff")), GrappleConfig.getClientConf().sounds.enderstaff_sound_volume * 0.5F, 1.0F);
//				Minecraft.getInstance().getSoundManager().play(new SimpleSound(ResourceLocation.fromNamespaceAndPath("grappling_hook_neo", "enderstaff"), SoundCategory.PLAYERS, GrappleConfig.getClientConf().sounds.enderstaff_sound_volume * 0.5F, 1.0F, false, 0, ISound.AttenuationType.NONE, player.getX(), player.getY(), player.getZ(), false));
				ClientProxyInterface.proxy.playSound(ResourceLocation.fromNamespaceAndPath("grappling_hook_neo", "enderstaff"), GrappleConfig.getClientConf().sounds.enderstaff_sound_volume * 0.5F);
			}
		}
	}
			...
			if (hasWallrunAbility(entity)) {
				if (!ClientSetup.key_jumpanddetach.isDown() && !Minecraft.getInstance().options.keyJump.isDown()) {
					...
					if (raytraceresult == null) {
						...
						return current_speed >= GrappleConfig.getConf().enchantments.wallrun.wallrun_min_speed;
					}
				}
			}
		}
	...
	public boolean wearingDoubleJumpEnchant(Entity entity) {
		...
		return hasDoubleJumpAbility(entity);
	}
	
	public static boolean isWearingSlidingEnchant(Entity entity) {
		return isWearingLongFallBoots(entity) || ModEnchantments.hasSlidingEnchantment(entity);
	}

	private static boolean hasWallrunAbility(Entity entity) {
		return isWearingLongFallBoots(entity) || ModEnchantments.hasWallrunEnchantment(entity);
	}

	private static boolean hasDoubleJumpAbility(Entity entity) {
		return isWearingLongFallBoots(entity) || ModEnchantments.hasDoubleJumpEnchantment(entity);
	}
	...
	private static boolean isWearingLongFallBoots(Entity entity) {
		...
		for (ItemStack stack : livingEntity.getArmorSlots()) {
			if (stack != null && stack.getItem() instanceof LongFallBoots) {
				return true;
			}
		}
		return false;
		...
		if (entity.onGround() && ClientSetup.key_slide.isDown()) {
			if (isWearingSlidingEnchant(entity)) {
				...
				if (controllers.containsKey(id)) {
					...
					if (controller instanceof AirFrictionController afc) {
						if (afc.wasSliding) {
							was_sliding = true;
						}
					}
				...
				return speed > GrappleConfig.getConf().enchantments.slide.sliding_end_min_speed && (was_sliding || speed > GrappleConfig.getConf().enchantments.slide.sliding_min_speed);
			}
		}
...
//		System.out.println(blockpos);
	...
	public static class RocketSound extends AbstractTickableSoundInstance {
		...
		protected RocketSound(GrappleController controller, SoundEvent p_i46532_1_, SoundSource p_i46532_2_) {
			super(p_i46532_1_, p_i46532_2_, RandomSource.create());
			this.looping = true;
	...
	public void startRocket(Player player, GrappleCustomization custom) {
		...
		RocketSound sound = new RocketSound(controller, SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath("grappling_hook_neo", "rocket")), SoundSource.PLAYERS);
