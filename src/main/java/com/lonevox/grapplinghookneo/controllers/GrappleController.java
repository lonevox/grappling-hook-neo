package com.lonevox.grapplinghookneo.controllers;
...
public class GrappleController {
	...
	public GrappleController(int grapplehookEntityId, int entityId, Level world, Vec pos, int controllerid, GrappleCustomization custom) {
		...
		if (grapplehookEntityId != -1) {
			...
			if (grapplehookEntity != null && grapplehookEntity.isAlive() && grapplehookEntity instanceof GrapplehookEntity) {
				...
				GrapplingHookNeo.LOGGER.warn("no hook entity");
				this.unattach();
			}
		}
	...
	public void unattach(boolean allowFollowupControllers) {
		...
		NetworkSetup.sendToServer(new GrappleEndMessage(this.entityId, this.grapplehookEntityIds));
...
//									vec pullxy = new vec(pull.x, 0, pull.z);
	...
	public void updateServerPos() {
		NetworkSetup.sendToServer(new PlayerMovementMessage(this.entityId, this.entity.position().x, this.entity.position().y, this.entity.position().z, this.entity.getDeltaMovement().x, this.entity.getDeltaMovement().y, this.entity.getDeltaMovement().z));
	}
