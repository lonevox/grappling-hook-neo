...
public class GrappleCustomization {
	...
	public enum upgradeCategories {
		...
		public String getName() {
			if (ClientProxyInterface.proxy != null) {
				return ClientProxyInterface.proxy.localize("grappling_hook_neo.upgradecategories." + this.nameUnlocalized);
			} else {
	...
	public void loadNBT(CompoundTag compound) {
		...
		if (compound.contains("crc32")) {
			...
			if (this.getChecksum() != recordedChecksum) {
				GrapplingHookNeo.LOGGER.error("Error checksum reading from NBT");
				this.setDefaults();
			}
		}
	}
	...
	public void readFromBuf(ByteBuf buf) {
		...
		if (this.getChecksum() != recordedChecksum) {
			GrapplingHookNeo.LOGGER.error("Error checksum reading from buffer");
			this.setDefaults();
		}
	}
